...
# Upload file to S3
run.cmd("aws s3 cp /tmp/{} s3://{}/Medical/{}".format(filename, run.config('BUCKET_FILES'), filename))

# Insert metadata into database
run.sql("INSERT INTO medical_files (name, url, size, date) VALUES (%s, %s, %s, %s);", name, "Medical/{}".format(filename), Path("/tmp/{}".format(filename)).stat().st_size, connix.now())
